...
def sim(enc, cls, memory_data_loader, test_data_loader, topk=500):
    ...
    plt.ylabel('The number of samples')
    plt.xlabel('Mean Cosine Similarity')
    plt.savefig("results/sim_test_train_enc.png")
    plt.close()
    data = [train_feature_bank_k.to('cpu').detach().numpy().copy(),test_feature_bank_k.to('cpu').detach().numpy().copy()]
    ks_result = kstest(train_feature_bank_k.to('cpu').detach().numpy().copy(),test_feature_bank_k.to('cpu').detach().numpy().copy(), alternative='two-sided', method='auto')
    plt.title(f'{ks_result.pvalue}')
    plt.hist(data[0], 30, alpha=0.6, density=True, label=olabels[0], stacked=False, range=(0.7, 1.0), color=color[0])
    plt.hist(data[1], 30, alpha=0.6, density=True, label=olabels[1], stacked=False, range=(0.7, 1.0), color=color[1])
    plt.legend()
    plt.ylabel('The number of samples')
    plt.xlabel('Mean Cosine Similarity')
    plt.savefig("results/sim_test_train_cls.png")
    plt.close()
